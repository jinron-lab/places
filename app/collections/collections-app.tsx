...
function CollectionsSidebar() {
  ...
  return <aside className="sidebar">
    ...
    <nav className="primary-nav" aria-label="Main navigation">
      ...
      <Link className="active" href="/collections"><span>▦</span> Collections <b>{journal.categories.length}</b></Link>
...
export function CollectionsIndex() {
  ...
  return <ResponsiveAppShell active="collections"><main className="collections-shell"><CollectionsSidebar /><section className="collections-panel"><div className="mobile-header"><Link className="brand" href="/"><span className="brand-mark">E</span><span>Explore</span></Link><button className="avatar">AJ</button></div><header className="collections-heading"><p className="eyebrow">MY COLLECTIONS</p><h1>Places, gathered by meaning.</h1><p>Every category becomes a collection of the memories you attached to it.</p></header>{journal.categories.length > 0 ? <div className="collection-grid">{journal.categories.map((category) => { const count = journal.entries.filter((entry) => entry.categoryIds.includes(category.id)).length; return <Link href={`/collections/${category.id}`} className="collection-card" key={category.id}><div className="collection-color" style={{ backgroundColor: category.color }}>{category.icon && <span>{category.icon}</span>}</div><div><h2>{category.name}</h2><p>{count} {count === 1 ? "journal entry" : "journal entries"}</p><span className="collection-card-action">View collection →</span></div><span className="card-arrow">›</span></Link>; })}</div> : <div className="collections-empty"><span>▦</span><h2>No collections yet</h2><p>Create a category while logging or editing a visit. It will appear here automatically.</p><Link href="/?mode=log">Log a place</Link></div>}</section><CollectionsMobileNav /></main></ResponsiveAppShell>;
}
